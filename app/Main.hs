...
checkOpts (Stdin:_) _ = evalLambadaTest =<< getContents
checkOpts (File f:opts) nonOpts = do
  readFile f >>= evalLambadaTest
  checkOpts opts nonOpts
checkOpts (Expr e:opts) nonOpts = evalLambadaTest e >> checkOpts opts nonOpts
...
repl env = do
  ...
  case words <$> line of
    ...
    Just ["quit"] -> return ()
    Just (x:"=":xs) ->
      case evalLambadaWith env (unwords xs) of
        Left err -> outputStrLn err >> repl env
        Right e -> repl (insertEnv x e env)
    Just line' ->
      case evalLambadaWith env (unwords line') of
        Left err -> outputStrLn err >> repl env
