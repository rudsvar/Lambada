...
-- along with a result.
newtype GeneralParser b a = P {
  runParser :: State b -> Either (State b) (a, State b)
}

-- | The `Functor` instance of `GeneralParser`,
-- letting you apply a function to the
-- result of a given parser.
instance Functor (GeneralParser b) where
  fmap f p = p >>= pure . f

-- | The `Applicative` instance of `GeneralParser`,
...
instance Applicative (GeneralParser b) where
  pure x = P $ \st -> Right (x, st)
  p <*> q = p >>= (<$> q)

-- | The `Alternative` instance of `GeneralParser`,
...
instance Alternative (GeneralParser b) where
  empty = P Left
  p <|> q = P $ \old ->
...
-- | The `Monad` instance of the `GeneralParser`,
...
instance Monad (GeneralParser b) where
  p >>= f = P $ \st ->
...
instance MonadPlus (GeneralParser b)
